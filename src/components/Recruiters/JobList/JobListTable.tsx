...
const JobTable: React.FC<Props> = ({ data, onRefresh }) => {
  ...
  const handleToggleStatus = async (id: string) => {
    ...
    toast.promise(openCloseJob(id), {
      ...
      error: (err) => {
        const errorMessage = err.response?.data?.error || "Failed to change job status";
        return `${errorMessage}`;
      },
    });
  };
            ...
            {data.map((job, index) => (
              ...
              >
                ...
                <td className="py-3 px-4">
                  <span
                    className={`inline-flex items-center justify-center w-20 py-1 rounded text-sm font-medium ${job.status === "closed"
                  ...
                  <div className="relative inline-block text-left">
                    ...
                    {openMenuId === job._id && (
                      <>
                        ...
                        {/* Dropdown Box: Dynamic positioning based on row index */}
                        <div className={`absolute right-0 z-40 w-36 bg-white border border-[#D6DDEB] rounded shadow-xl py-1 
